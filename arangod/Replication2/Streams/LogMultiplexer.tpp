...
struct LogMultiplexerImplementation
  ...
  auto insertInternal(T const& t) -> LogIndex {
    ...
    auto [index, waitForIndex, outerAction] = std::invoke([&] {
      std::unique_lock guard(_insertMutex);
      // First write to replicated log - note that insert could trigger a
      // waitFor to be resolved. Therefore, we should hold the lock.
      // TODO add a deferred insert, that hands out the deferred action
      //    we then postpone this action until we release the insertMutex.
      auto [insertIndex, action] = _interface->insertWithDeferredReplication(
          LogPayload(std::move(serialized)), false);

      return this->_guardedData.doUnderLock([&, &insertIndex = insertIndex,
                                             &action = action](auto& self) {
        ...
        return std::make_tuple(insertIndex, self.checkWaitFor(),
                               std::move(action));
      });
    });
    // resolve promises here
    outerAction.fire();
