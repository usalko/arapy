...
TEST_F(LogContextTest, pushValues_adds_shared_values_to_context_and_returns_an_EntryPtr) {
  unsigned cnt = 0;
  LogContext::OverloadVisitor countingVisitor([&cnt](std::string_view, auto&&) { ++cnt; });
  auto values = LogContext::makeValue().with<LogKey1>("blubb").share();

  LogContext::EntryPtr entry1 = LogContext::Current::pushValues(values);
  LogContext::EntryPtr entry2 = LogContext::Current::pushValues(LogContext::makeValue().with<LogKey2>(42));
