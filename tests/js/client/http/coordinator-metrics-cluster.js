...
/*global fail, assertUndefined, assertEqual, assertNotEqual, assertTrue, assertFalse, assertNull, print*/
...
function CoordinatorMetricsTestSuite() {
    return {
        setUpAll: function () {
            ...
            db._create("foo1", {"replication": 3, "numberOfShards": 9});
            db._create("foo2", {"replication": 2, "numberOfShards": 12});
            db._create("foo3", {"replication": 1, "numberOfShards": 1});
            let foo1_values = [];
...
////////////////////////////////////////////////////////////////////////////////
        ...
        testIResearchLinkStats: function () {
            ...
            assertEqual(metrics["arangosearch_num_live_docs"]["foo1"], 100);
            assertEqual(metrics["arangosearch_num_live_docs"]["foo2"], 101);
            assertEqual(metrics["arangosearch_num_live_docs"]["foo3"], 102);

            assertEqual(metrics["arangosearch_num_segments"]["foo1"], 9);
            assertEqual(metrics["arangosearch_num_segments"]["foo2"], 12);
            assertEqual(metrics["arangosearch_num_segments"]["foo3"], 1);

            assertEqual(metrics["arangosearch_num_files"]["foo1"], 54);
            assertEqual(metrics["arangosearch_num_files"]["foo2"], 72);
            assertEqual(metrics["arangosearch_num_files"]["foo3"], 6);

            assertTrue(metrics["arangosearch_index_size"]["foo1"] > 0);
            assertTrue(metrics["arangosearch_index_size"]["foo2"] > 0);
            assertTrue(metrics["arangosearch_index_size"]["foo3"] > 0);
        }
    };
}
